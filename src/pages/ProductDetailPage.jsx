...
import { X, Heart, ShoppingBag, MessageSquare, Sparkles, ArrowLeft, Star, Share2, ChevronLeft, ChevronRight, Truck, RefreshCw, ShieldCheck, CheckCircle2 } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    autoScrollRef.current = setInterval(() => {
      setCurrentImageIndex(prev => {
        ...
        const next = (prev + 1) % (imagesCount + 1); // Go towards the clone
        ...
        if (scrollRef.current && window.innerWidth < 768) {
          ...
          if (next === imagesCount) {
            ...
            scrollRef.current.scrollTo({ left: next * scrollWidth, behavior: 'smooth' });
            
            // Then SILENTLY jump back to index 0 after animation finishes
            setTimeout(() => {
                  ...
                  <div style={{ 
                    ...
                    {(product.images?.length > 0 ? product.images : [product.image]).map((_, idx) => (
                      <div 
                        key={idx} 
                        style={{ 
                          width: (currentImageIndex % (product.images?.length || 1)) === idx ? '24px' : '6px', 
                          height: '6px', 
                          borderRadius: '4px', 
                          background: (currentImageIndex % (product.images?.length || 1)) === idx ? 'var(--primary)' : 'rgba(0,0,0,0.2)', 
            ...
            <motion.div initial="hidden" animate="visible" transition={{ staggerChildren: 0.1 }}>
              ...
              <motion.div variants={itemVariants} style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{ 
                    flex: 1, 
                    minWidth: '160px', 
                    padding: window.innerWidth < 768 ? '0.8rem' : '1.1rem 1.5rem', 
                    background: product.stock === 0 ? '#ccc' : 'var(--primary)', 
                    color: 'white', 
                    borderRadius: '10px', 
                    border: 'none', 
                    fontWeight: 700, 
                    fontSize: window.innerWidth < 768 ? '0.8rem' : '0.92rem', 
                    display: 'flex', 
                    alignItems: 'center', 
                ...
                <button
                  ...
                  style={{ padding: window.innerWidth < 768 ? '0.8rem' : '1rem 1.3rem', background: '#fff0f0', borderRadius: '10px', border: 'none', color: 'var(--primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
              ...
              <motion.button
                ...
                style={{ 
                  width: '100%', 
                  marginTop: '1rem', 
                  padding: window.innerWidth < 768 ? '0.8rem 1.2rem' : '1rem 2rem', 
              ...
              {/* Amazon/Flipkart Premium Features Section */}
              <motion.div variants={itemVariants} style={{ marginTop: '2.5rem', paddingTop: '2rem', borderTop: '1px solid #f0f0f0', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }}>
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'flex-start' }}>
                  <div style={{ padding: '0.6rem', borderRadius: '10px', background: '#f5f7ff', color: '#3b82f6' }}><Truck size={20} /></div>
                  <div>
                    <p style={{ fontWeight: 800, fontSize: '0.75rem', color: 'var(--secondary)' }}>7-Day Delivery</p>
                    <p style={{ fontSize: '0.65rem', color: '#888', marginTop: '0.2rem' }}>Swift & trackable artisanal shipping </p>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'flex-start' }}>
                  <div style={{ padding: '0.6rem', borderRadius: '10px', background: '#f0fff4', color: '#22c55e' }}><RefreshCw size={20} /></div>
                  <div>
                    <p style={{ fontWeight: 800, fontSize: '0.75rem', color: 'var(--secondary)' }}>Easy Returns</p>
                    <p style={{ fontSize: '0.65rem', color: '#888', marginTop: '0.2rem' }}>7-day hassle-free return policy</p>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'flex-start' }}>
                  <div style={{ padding: '0.6rem', borderRadius: '10px', background: '#fff9f0', color: '#f59e0b' }}><ShieldCheck size={20} /></div>
                  <div>
                    <p style={{ fontWeight: 800, fontSize: '0.75rem', color: 'var(--secondary)' }}>Safe & Secure</p>
                    <p style={{ fontSize: '0.65rem', color: '#888', marginTop: '0.2rem' }}>100% encrypted & secure payments</p>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'flex-start' }}>
                  <div style={{ padding: '0.6rem', borderRadius: '10px', background: '#fdf2f8', color: '#ec4899' }}><CheckCircle2 size={20} /></div>
                  <div>
                    <p style={{ fontWeight: 800, fontSize: '0.75rem', color: 'var(--secondary)' }}>Artisanal Quality</p>
                    <p style={{ fontSize: '0.65rem', color: '#888', marginTop: '0.2rem' }}>Verified handcrafted excellence</p>
                  </div>
                </div>
              </motion.div>
            </motion.div>
